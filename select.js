...
export default class Select {
  ...
  get selectedOptionIndex() {
    return this.options.findIndex((x) => x.selected);
  }
  selectValue(value) {
...
function setupCustomElement(select) {
  ...
  select.options.forEach((option) => {
    console.log("enter foreach options==>", option);
    const optionElement = document.createElement("li");
    optionElement.classList.add("custom-select-option");
    optionElement.classList.toggle("selected", option.selected);
    optionElement.innerText = option.label;
  ...
  select.customElement.addEventListener("keydown", (e) => {
    ...
    default:
     ...
     const searchedOption = select.options.find(x=>x.label.toLowerCase().startsWith(searchTerm))
...
function getFormattedOptions(optionsElements) {
  return [...optionsElements].map((optionElem) => {
    return {
